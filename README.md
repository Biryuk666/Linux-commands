...
- [Команды Linux](#команды-linux)
  ...
  - [Создание файлов при помощи команд `cat`, `touch`, `gedit` и `vi редактора`](#создание-файлов-при-помощи-команд-cat-touch-gedit-и-vi-редактора)
  ...
  - [Просмотр содержимого файлов при помощи команд `less` и `more`](#просмотр-содержимого-файлов-при-помощи-команд-less-и-more)
  - [Создание скрытых файлов и каталогов](#создание-скрытых-файлов-и-каталогов)
...
- Вывести рабочий каталог(текущую дерикторию): `pwd`
...
- Перейти в домашний каталог: `cd` или `cd ~`
...
- `opt` каталог: `opt` означает опциональный (optional). Он содержит все установочные файлы программного обеспечения сторонних производителей.
...
- Вывести ход процесса при удалении `v`: `rm -rv имя_каталога`
...
## Создание файлов при помощи команд `cat`, `touch`, `gedit` и `vi редактора`
...
- Отобразить все содержимое файла, за исключением последних 5 строк: `head -n -5 имя_файла`.
...
## Просмотр содержимого файлов при помощи команд `less` и `more`

- Отобразить содержимое файла страница за страницей: `more имя_файла`. Нажмите `enter` для отображения следующей строки, `пробел` для отображения следующей страницы, `q` для выхода.
- Отобразить больше информации о файле: `more -d имя_файла`. 
Используя команду `more` для просмотра содержимого файлов, мы можем перемещаться по страницам только вперед.
- Отобразить содержимое файла страница за страницей с возможностью перемещаться по ним вперед и назад: `less имя_файла`. Нажмите `d` для отображения следующей страницы, где `d` означает вниз (down), и  `b` to для отображения предыдущей страницы, где `b` означает назад (backward).

## Создание скрытых файлов и каталогов

Если название любого файла или каталога начинается с `.`, то такой файл является скрытым.
- Создать скрытый файл: `touch .имя_файла`, `cat > .имя файла` и т.д.
- Отобразить скрытые файлы: `ls -a`
- Создать скрытый каталог: `mkdir .имя_каталога`
- Преобразовать скрытый файл в обычный: `mv .имя_файла имя_файла`.
